...
begin_with_star_match = re.search(r'const beginWithStar = (\[.*?\]);', js_content, re.DOTALL)

if begin_with_star_match:
    # 提取数组字符串并处理
    begin_with_star_str = begin_with_star_match.group(1)
    
    # 去掉数组中每个 URL 的星号（*）
    begin_with_star_str = re.sub(r'\*', '', begin_with_star_str)
    
    # 将处理后的字符串转换为 Python 列表
    try:
        begin_with_star_host = ast.literal_eval(begin_with_star_str)
...
without_star_match = re.search(r'const\s+beginWithoutStar\s*=\s*\[([^\]]*)\];', js_content)

if without_star_match:
    without_star_host = re.findall(r'"([^"]+)"', without_star_match.group(1))
    without_star_host = sorted(without_star_host, key=len)


merged_result = set(begin_with_star_host+without_star_host)  # 使用集合来去重
formatted_urls = [f'"{url}",' for url in merged_result if url != "google.com"]
merged_text = '\n'.join(sorted(formatted_urls))  # 使用 sorted() 对内容进行排序，便于查看
